...
mymodel = load_model('bi-rnn.h5')
tokenizer = pickle.load(open("bi-rnn.pickle", 'rb'))
...
for i in range(len(lines)):
    ...
    if (len(temp)>0) and ("?" in temp) and ("=" in temp) and (temp !=" "):
        # url = temp[temp.index("T /")+len("T /"):temp.index("?")]
        ...
        # res_url = predict_sqli_attack(url)
        # if res_url > 0 :
        #     append_log(lines[i], url, res_url)
